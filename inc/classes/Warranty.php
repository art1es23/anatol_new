...
if (!class_exists('Warranty')) {
  class Warranty
  {
    ...
    function update_warranty_user_fields($data,$user_id)
    {
      ...
      update_field("address_line_one", $data['lane'], $user_id);
      update_field("address_line_two", $data['cf_1386'], $user_id);
      update_field("city_warranty", $data['city'], $user_id);
      update_field("state_warranty", $data['cf_1137'], $user_id);
      update_field("zip_code_warranty", $data['code'], $user_id);
      update_field("phone_warranty", $data['phone'], $user_id);
      update_field("email_warranty", $data['email'], $user_id);

      
      update_field("equipment_purchased_warranty", $data['cf_1139'],$user_id);
      update_field("serial_number_warranty", $data['cf_945'],$user_id);

      update_field("description", $data['description'], $user_id);
