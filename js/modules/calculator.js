jQuery(function ($) {
  ...
  const loadone = () =>
    document.querySelector(".register-form .btn-one").classList.add("load");
  const loadtwo = () =>
  ...
  function userRoI(wrapper) {
    ...
    if (document.cookie.indexOf('roi_user_id":') !== -1) {
      key = document.cookie.slice(
        document.cookie.indexOf('roi_user_id":') + 'roi_user_id":'.length,
        document.cookie.indexOf("}", document.cookie.indexOf('roi_user_id":'))
      );
    }
  ...
  function setCookies(data) {
    document.cookie = "ROI_user=" + data + "; max-age=" + 604800 + "; path=/";
  ...
  function makeCalculation(formData) {
    ...
    $.post(
      ...
      function (data, status) {
        ...
        const getShowElement = (className) =>
          document.querySelector(className).querySelectorAll(".val");

        const showResults = (el, key) =>
          el.forEach(
            (item, idx) => (item.textContent = data[key][item.dataset.value])
          );
        ...
        showResults(
          getShowElement("#total-savings-year"),
          "total_savings_year"
        );
